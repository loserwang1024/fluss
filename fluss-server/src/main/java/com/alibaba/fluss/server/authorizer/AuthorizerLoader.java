...
public class AuthorizerLoader {
    ...
    /** Load authorizer. */
    public static Authorizer createAuthorizer(
        ...
        Collection<Supplier<Iterator<AuthorizePlugin>>> pluginSuppliers = new ArrayList<>(2);
        pluginSuppliers.add(() -> ServiceLoader.load(AuthorizePlugin.class).iterator());

        if (pluginManager != null) {
            pluginSuppliers.add(() -> pluginManager.load(AuthorizePlugin.class));
        }

        List<AuthorizePlugin> matchingPlugins = new ArrayList<>();
        for (Supplier<Iterator<AuthorizePlugin>> pluginSupplier : pluginSuppliers) {
            Iterator<AuthorizePlugin> plugins = pluginSupplier.get();
            while (plugins.hasNext()) {
                AuthorizePlugin plugin = plugins.next();
